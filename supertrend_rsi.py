...
class SuperTrendRSIStrategy:
    ...
    def _compute(
        ...
        if curr_dir == 1 and prev_dir == -1:
            confidence_factors.append(0.40)  # ST flip is strong
        elif curr_dir == 1:
            confidence_factors.append(0.20)  # Already bullish
        ...
        if curr_dir == 1 and (rsi_bull or (rsi_mid_bull and prev_dir == -1)):
            ...
            if htf_bull:
                ...
                confidence += 0.10
            # Taker buy ratio
            if "taker_ratio" in df.columns and df["taker_ratio"].iloc[-1] > 0.55:
                confidence += 0.10
        ...
        # ----- SHORT -----
            ...
            if htf_bear:
                ...
                confidence += 0.10
            if "taker_ratio" in df.columns and df["taker_ratio"].iloc[-1] < 0.45:
                confidence += 0.10
            confidence = min(confidence, 1.0)
